...
#define FB1_1 (0x2A00000+320*240*8)
#define FBX_L (320*240*2)
#define FBX32_L (320*240*4)
...
unsigned int *fb1_32bit; /* current buffer (32 bit) */
...
/* Sound related */
static pthread_t wiz_sound_thread;
unsigned short  wiz_sound_buffer[4+ ( (44100*2) *8) ];
volatile short *pOutput[8]; 
volatile int CurrentSoundBank=0;
static volatile unsigned long  wiz_sound_pausei, wiz_exit;

static void lc_setfb(int layer, unsigned short *set_to);
static void lc_flipfb(int layer,int single);
static void lc_setlayer(int layer, int onoff, int alpha, int invert, int trans, unsigned int mode);
...
static void lc_flipfb(int layer,int single)
{
    ...
    if (single)
    {
    	if(layer == 0) {
			...
			fb1_32bit = (unsigned int *)fb1_16bit;
    	}
    }
    /* double buffer */
    else
...
int wiz_enable_back_layer(int enabled, int bpp)
{
	...
	{
		lc_setlayer(1, 1, 0, 0, 0, XRGB8888);
	}
	else
	{
...
int wiz_init(int bpp, int rate, int bits, int stereo)
{
	...
	fb1_0 = (unsigned short *)upper_take(FB1_0,FBX32_L); // do not use video buffer memory
	fb1_1 = (unsigned short *)upper_take(FB1_1,FBX32_L); // do not use video buffer memory
	upper_take(FB1_1+FBX32_L,(0x3000000-0x2A00000)-(FBX_L*2+FBX32_L*2)); // do not use kernel memory
	
    /* assign initial framebuffers */
	fb0_16bit = fb0_1; fb0_8bit=(unsigned char *)fb0_16bit;
	fb1_16bit = fb1_1; fb1_8bit=(unsigned char *)fb1_16bit; fb1_32bit=(unsigned int *)fb1_16bit;

	/* clear framebuffers */
	memset((void*)fb0_0, 0x00, FBX_L);
	memset((void*)fb0_1, 0x00, FBX_L);
	memset((void*)fb1_0, 0x00, FBX32_L);
	memset((void*)fb1_1, 0x00, FBX32_L);

    /* set screen orientation */
	lc_screensize(320, 240); 
	lc_setbgcol(0x0); /* set default background colour */
	lc_layerpos(0, 0, 0, 319, 239);	/* set default layer positions */
	lc_layerpos(1, 0, 0, 319, 239);
	
	if (bpp==16)
	{
	    lc_setlayer(0, 1, 0, 0, 1, RGB565); /* set default layer settings */
	    lc_setlayer(1, 0, 0, 0, 0, RGB565);
	}
	else
	{
	    lc_setlayer(0, 1, 0, 0, 1, PTRGB565); /* set default layer settings */
	    lc_setlayer(1, 0, 0, 0, 0, PTRGB565);
        int i;
        for (i=0; i<256; i++)
        {
            wiz_video_color8(i,0,0,0);
...
void wiz_deinit(void)
{
	...
	wiz_set_clock(533);
  	...
  	memset(fb0_16bit, 0, FBX_L); wiz_video_flip(0);
  	memset(fb0_16bit, 0, FBX_L); wiz_video_flip(0);
	wiz_video_flip_single(0);
...
{
#ifdef DEBUG
	printf ("Setting Wiz Speed: %d Mhz\n", speed);
#endif
	unsigned  long v;
...
void wiz_video_flip(int layer)
{
    lc_flipfb(layer,0);
}

void wiz_video_flip_single(int layer)
{
    lc_flipfb(layer,1);
}
...
void wiz_video_setpalette(void)
{
    ...
    for (i=0; i<256; i++)
    {
        ...
        {
            MLCPALETTE0 = i<<24 | wiz_video_RGB_palette[i].color;
            wiz_video_RGB_palette[i].dirty = 0;
        }
    }
    MLCCONTROL0 |= 0x10; // Apply changes
	//lc_dirtylayer(1);
}
...
void wiz_sound_pause (int yes)
{
	wiz_sound_pausei=yes;
}

static void *wiz_sound_play (void)
{
	while (!wiz_exit)
	{
		if (!wiz_sound_pausei)
		{
			...
			write (wiz_dev[1], (void *) pOutput[CurrentSoundBank], wiz_sound_buffer[1]);
...
void wiz_sound_init (int rate, int bits, int stereo, int Hz)
{
	...
	if (!wiz_dev[1]) wiz_dev[1] = open ("/dev/dsp", O_WRONLY);

	//set sound
	ioctl (wiz_dev[1], SNDCTL_DSP_SPEED,  &rate);
	ioctl (wiz_dev[1], SNDCTL_DSP_SETFMT, &bits);
	ioctl (wiz_dev[1], SNDCTL_DSP_STEREO, &stereo);
  ...
  //int frag = 0x80000|7; // Original
  //int frag=0xf0008;

  //int frag=0x0002000F; // DrMD

  //ioctl(wiz_dev[1], SNDCTL_DSP_SETFRAGMENT, &frag);
  //ioctl(wiz_dev[1], SNDCTL_DSP_GETOSPACE, &abi);

	unsigned int bufferStart=0;
	wiz_sound_buffer[1]= (wiz_sound_buffer[0]= (rate/Hz) ) << (stereo + (bits==16) );
	wiz_sound_buffer[2]= (1000000000/Hz) &0xFFFF;
	wiz_sound_buffer[3]= (1000000000/Hz) >>16;

#ifdef DEBUG
	printf ("wiz_sound_buffer[0]: %d\n", wiz_sound_buffer[0]);
	printf ("wiz_sound_buffer[1]: %d\n", wiz_sound_buffer[1]);
	printf ("wiz_sound_buffer[2]: %d\n", wiz_sound_buffer[2]);
	printf ("wiz_sound_buffer[3]: %d\n", wiz_sound_buffer[3]);
#endif
	
	bufferStart= &wiz_sound_buffer[4];
	pOutput[0] = (short*) bufferStart+ (0*wiz_sound_buffer[1]);
	pOutput[1] = (short*) bufferStart+ (1*wiz_sound_buffer[1]);
	pOutput[2] = (short*) bufferStart+ (2*wiz_sound_buffer[1]);
	pOutput[3] = (short*) bufferStart+ (3*wiz_sound_buffer[1]);
	pOutput[4] = (short*) bufferStart+ (4*wiz_sound_buffer[1]);
	pOutput[5] = (short*) bufferStart+ (5*wiz_sound_buffer[1]);
	pOutput[6] = (short*) bufferStart+ (6*wiz_sound_buffer[1]);
	pOutput[7] = (short*) bufferStart+ (7*wiz_sound_buffer[1]);

	int i;

	for (i=0; i< (wiz_sound_buffer[1]*8); i++)
	{
		wiz_sound_buffer[4+i] = 0;
	}
	...
	if (!wiz_sound_thread)
	{
		pthread_create ( &wiz_sound_thread, NULL, wiz_sound_play, NULL);
	}
}
